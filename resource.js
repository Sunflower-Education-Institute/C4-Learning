(function () {
  ...
  function renderReviews(resource) {
    const items = reviews.filter(review => review.sourceId === resource.sourceId).slice(0, 4);
    ...
    return items.map(item => {
      ...
      return `<details class="review"><summary><span class="reviewer-line"><strong>${escapeHtml(displayName)}</strong>${item.rating ? `<span class="review-rating">★ ${escapeHtml(item.rating)}</span>` : ""}</span><span class="reviewer-badges">${badges}</span><small>${t("readReview")}</small></summary><p>${escapeHtml(item.reviewSummary)}</p></details>`;
    }).join("");
  }
  ...
  function render() {
    ...
    const visibleTags = allTags(resource).slice(0, 4);
    const reviewCount = reviews.filter(review => review.sourceId === resource.sourceId).length;
      ...
      <article class="compact-detail-shell">
        <section class="compact-detail-hero">
          ...
          <div class="compact-detail-intro">
            <div class="compact-title-row"><div><p class="eyebrow">${escapeHtml(resource.sourceType || "Learning resource")}</p><h1>${escapeHtml(resource.resourceName)}</h1></div><button class="save-detail-button ${saved ? "saved" : ""}" type="button" data-save-detail aria-pressed="${saved}">${saved ? "★" : "☆"} ${saved ? t("saved") : t("save")}</button></div>
            ...
            <div class="detail-grid compact-metadata">
              ...
              <div class="detail-item"><span>${t("type")}</span><strong>${escapeHtml(resource.sourceType || "—")}</strong></div>
              <div class="detail-item"><span>${t("score")}</span><strong>${Number.isFinite(Number(resource.initialSeeScore)) ? `★ ${escapeHtml(resource.initialSeeScore)} / 5` : "—"}</strong></div>
              <div class="detail-item"><span>${t("cost")}</span><strong>${escapeHtml(resource.originalCostEstimateUsd || "—")}</strong></div>
            </div>
        ...
        <div class="detail-dashboard">
          ...
          <section class="compact-review-panel"><div class="panel-heading"><div><p class="eyebrow">Community</p><h2>${t("reviews")}</h2><p>${reviewCountLabel(reviewCount)}</p></div></div>${renderReviews(resource)}</section>
        </div>
      </article>`;
