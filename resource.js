(function () {
  ...
  const copy = {
    en: { back: "Back to all resources", age: "Age range", type: "Resource type", score: "SEE review score", cost: "Current pricing", overview: "Full resource overview", reviews: "Parent and educator reviews", reviewAverage: "Review average", noReviews: "No reviews are available yet.", visit: "Visit official source", save: "Save resource", saved: "Saved", unavailable: "Resource not found", unavailableBody: "This resource may have moved or the link may be incomplete.", summaryEyebrow: "Editorial summary", aiTitle: "AI-assisted review summary", aiSubtitle: "Prepared from published reviews", aiPending: "A bilingual review summary is waiting for editorial preparation.", pros: "What reviewers valued", considerations: "What to consider", basedOn: "Based on", publishedReview: "published review", publishedReviews: "published reviews", summaryNote: "AI-assisted summary · Editorial review recommended", readReview: "Read full review" },
    zh: { back: "返回全部资源", age: "适用年龄", type: "资源类型", score: "SEE 评审评分", cost: "当前价格信息", overview: "完整资源介绍", reviews: "家长与教育者评价", reviewAverage: "评论平均分", noReviews: "目前还没有评价。", visit: "访问原始网站", save: "收藏资源", saved: "已收藏", unavailable: "没有找到该资源", unavailableBody: "该资源可能已移动，或者链接不完整。", summaryEyebrow: "编辑汇总", aiTitle: "AI 辅助生成的评论汇总", aiSubtitle: "根据已发布评论整理", aiPending: "该资源的双语评论汇总仍在等待编辑整理。", pros: "评论者认可的方面", considerations: "需要考虑的方面", basedOn: "依据", publishedReviews: "条已发布评论", summaryNote: "AI 辅助汇总 · 建议人工复核", reviewCount: "条已发布评论", readReview: "阅读完整评论" }
  };
  ...
  function renderAiSummary(resource) {
    ...
    if (!summary) return `<section class="ai-summary-panel pending"><div class="panel-heading"><div><p class="eyebrow">${t("summaryEyebrow")}</p><h2>${t("aiTitle")}</h2><p>${t("aiSubtitle")}</p></div></div><p>${t("aiPending")}</p></section>`;
    const content = summary.content[language === "zh" ? "zh-Hans" : "en"];
    return `<section class="ai-summary-panel"><div class="panel-heading"><div><p class="eyebrow">${t("summaryEyebrow")}</p><h2>${t("aiTitle")}</h2><p>${t("aiSubtitle")} · ${t("basedOn")} ${reviewCountLabel(summary.basedOnReviewCount)}</p></div><span class="ai-spark" aria-hidden="true">✦</span></div><p class="ai-overview">${escapeHtml(content.overview)}</p><div class="ai-points"><div><h3>✓ ${t("pros")}</h3><ul>${content.pros.map(item => `<li>${escapeHtml(item)}</li>`).join("")}</ul></div><div><h3>△ ${t("considerations")}</h3><ul>${content.considerations.map(item => `<li>${escapeHtml(item)}</li>`).join("")}</ul></div></div><small class="ai-note">${t("summaryNote")} · ${escapeHtml(summary.generatedAt)}</small></section>`;
  }
